...
/// <see cref="TeamsClient"/>, and <see cref="OnedriveforbusinessClient"/> from the DirectClient SDK.
/// </summary>
/// <remarks>
...
public class ConnectorFunctions
{
    ...
    /// <summary>
    /// Lists files in the root of the user's OneDrive for Business using the generated <see cref="OnedriveforbusinessClient"/>.
    /// </summary>
    /// <param name="request">The HTTP request.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    ...
    /// <summary>
    /// Lists files in a specific OneDrive folder using the generated <see cref="OnedriveforbusinessClient"/>.
    /// </summary>
    /// <remarks>
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                input = JsonSerializer.Deserialize<UploadOneDriveFileRequest>(body, ConnectorFunctions.JsonOptions);
            }
            catch (JsonException ex)
            {
                this._logger.LogError(ex, "Invalid JSON in request body: '{Message}'.", ex.Message);
    ...
    /// <summary>
    /// Creates a sharing link for a OneDrive file using the generated <see cref="OnedriveforbusinessClient"/>.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                input = JsonSerializer.Deserialize<CreateShareLinkRequest>(body, ConnectorFunctions.JsonOptions);
            }
            catch (JsonException ex)
            {
                this._logger.LogError(ex, "Invalid JSON in request body: '{Message}'.", ex.Message);
    ...
    /// <remarks>
    /// <para>
    /// The OneDrive connector has two variants of file triggers:
    /// </para>
    /// <list type="bullet">
    /// <item>
    ...
    /// </item>
    /// <item>
    /// <term>OnNewFileV2 / OnUpdatedFileV2</term>
    /// <description>
    /// Returns the actual file content as raw bytes (binary). The AI Gateway POSTs
    /// the file bytes directly in the request body. This sample demonstrates how to
    /// detect and handle binary trigger payloads that are not JSON.
    /// </description>
    /// </item>
    /// </list>
    /// <para>
    /// This callback handles both variants: it first attempts JSON deserialization for the
    /// properties-only triggers, and falls back to binary handling for file-content triggers.
    /// </para>
    /// </remarks>
    /// <param name="request">The HTTP request containing the trigger payload.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (contentLength > ConnectorFunctions.MaxTriggerCallbackBodySize ||
                (contentLength < 0 && request.Body.CanSeek && request.Body.Length > ConnectorFunctions.MaxTriggerCallbackBodySize))
            {
                this._logger.LogWarning("OneDriveTriggerCallback: Payload too large. Rejecting.");
            ...
            if (isJsonPayload)
            {
                ...
                if (charsRead > ConnectorFunctions.MaxTriggerCallbackBodySize)
                {
                    this._logger.LogWarning("OneDriveTriggerCallback: JSON payload too large. Rejecting.");
            ...
            else
            {
                ...
                await response
                    ...
                    {
                        success = true,
                        message = "Trigger callback received (binary file content).",
                        receivedAt = DateTime.UtcNow,
