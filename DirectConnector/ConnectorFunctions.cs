...
public class ConnectorFunctions
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            SmtpSendEmailRequest? input;
            try
            {
                input = JsonSerializer.Deserialize<SmtpSendEmailRequest>(body, ConnectorFunctions.JsonOptions);
            }
            catch (JsonException ex)
            {
                this._logger.LogError(ex, "Invalid JSON in request body: '{Message}'.", ex.Message);
            ...
            if (input == null || string.IsNullOrEmpty(input.To) || string.IsNullOrEmpty(input.From))
            {
                var badRequest = request.CreateResponse(HttpStatusCode.BadRequest);
                await badRequest
                    .WriteAsJsonAsync(new { error = "Invalid request body - 'from' and 'to' are required." })
                    .ConfigureAwait(continueOnCapturedContext: false);
                return badRequest;
            }
